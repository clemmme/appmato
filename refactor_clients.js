...
// 1. Add React and useCallback/useMemo
code = code.replace("import { useState } from 'react';", "import React, { useState, useMemo, useCallback } from 'react';");

// 2. Wrap getRegimeLabel and getTotalFee in useCallback
code = code.replace(
...
// 3. Wrap filteredClients in useMemo
code = code.replace(
  `  const filteredClients = sortWithFavorites(
    clients.filter(c => {
...
`;
code = code.replace("export function ClientsView(", clientCardComponent);


// 5. Replace inside grid
const oldGridRegex = /\{filteredClients\.map\(client => \(\s*<div\s*key=\{client\.id\}[\s\S]*?<\/div>\n\s*\)\)\}/;
const newGrid = \`{filteredClients.map(client => (
            <ClientCard
              key={client.id}
              client={client}
